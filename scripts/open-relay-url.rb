...
OptionParser.new do |parser|
  parser.banner = "Usage: scripts/open-relay-url.rb [--demo|--scheme SCHEME] ENDPOINT_URL [simulator-device]"
...
relay_url = ARGV.fetch(0) do
  warn "Usage: scripts/open-relay-url.rb [--demo|--scheme SCHEME] ENDPOINT_URL [simulator-device]"
  exit 1
end
device = ARGV.fetch(1, "booted")

deep_link = "#{options.fetch(:scheme)}://network/relay?url=#{URI.encode_www_form_component(relay_url)}"
