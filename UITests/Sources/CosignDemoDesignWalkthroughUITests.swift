...
final class CosignDemoDesignWalkthroughUITests: XCTestCase {
    ...
    func testDevnetWalkthroughAgainstLiveRelay() throws {
        ...
        if coSign.waitForExistence(timeout: 8) {
            coSign.tap()
            if app.buttons["proposal-signing-hold-button"].waitForExistence(timeout: 8) {
    ...
    private func tapButton(_ identifier: String) {
        ...
        XCTAssertTrue(button.waitForExistence(timeout: 8), "Missing button \(identifier)")
        button.tap()
    }
    ...
    private func typeText(_ identifier: String, _ text: String) {
        ...
        XCTAssertTrue(textField.waitForExistence(timeout: 8), "Missing text field \(identifier)")
        textField.tap()
    ...
    private func waitForButton(_ identifier: String) {
        XCTAssertTrue(app.buttons[identifier].waitForExistence(timeout: 8), "Missing button \(identifier)")
    }

    private func waitForText(_ label: String) {
        XCTAssertTrue(app.staticTexts[label].waitForExistence(timeout: 8), "Missing text \(label)")
    }

    private func waitForScreen(_ identifier: String) {
        let element = app.descendants(matching: .any)[identifier]
        XCTAssertTrue(element.waitForExistence(timeout: 8), "Missing screen \(identifier)")
    }
